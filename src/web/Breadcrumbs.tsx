...
import type { Conversation, Member, Report, Work, WorkMessage } from '../shared/company-contracts'
import { api } from './api'
import { breadcrumbPoints, pageName, type ReturnPoint } from './navigation'
  ...
  useEffect(() => {
    ...
    const resolve = async (point: ReturnPoint) => {
      ...
      if (id && section === 'assistant')
        label = (await api<Conversation>(`/conversations/${id}`, { signal: controller.signal }))
          .title
      if (id && section === 'work') {
        const item = await api<Work>(`/work-items/${id}`, { signal: controller.signal })
        label = item.title
        ownerId = item.ownerId
      }
      if (id && section === 'reports') {
        const item = await api<Report>(`/reports/${id}`, { signal: controller.signal })
        label = item.period + (item.kind === 'daily' ? '日报' : '周报')
        ownerId = item.ownerId
      }
      if (id && section === 'messages') {
        const item = await api<WorkMessage>(`/messages/${id}`, { signal: controller.signal })
        ownerId = item.ownerId
