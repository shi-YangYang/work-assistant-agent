...
from paa_server.agent.operations import execute as actions_execute
...
from paa_server.modules.reports.sources import report_fact_basis as actions_report_fact_basis
from paa_server.security.access import employee as business_employee, receipt as business_receipt, remember as business_remember, resolve as business_resolve, scope as business_scope
...
async def execute_business_action(step: int, action: Literal['create_work', 'update_work', 'delete_work', 'generate_report', 'edit_report', 'submit_report', 'delete_report'], runtime: ToolRuntime[RunContext], target_id: str = '', expected_revision: int = 0, changes: dict | None = None, report_kind: Literal['daily', 'weekly'] = 'daily', report_date: str = '', obligation_id: str = '', source_tokens: list[str] | None = None, submit_after: bool = False, requires_step: int | None = None) -> str:
    ...
    """
    try:
        result = await actions_execute(runtime.context, step=step, action=action, target_id=target_id, expected_revision=expected_revision, changes=changes, report_kind=report_kind, report_date=report_date, obligation_id=obligation_id, source_tokens=source_tokens, submit_after=submit_after, requires_step=requires_step)
        ...
        try:
            ...
            for report in reports[:10]:
                ...
                if actor.id == report.owner_id and (report_id or len(reports) == 1):
                    items[-1]['sourceFacts'] = await actions_report_fact_basis(db, actor, report)
...
async def query_report_obligations(runtime: ToolRuntime[RunContext], kind: Literal['daily', 'weekly'] = 'daily', status: Literal['', 'pending', 'overdue', 'submitted', 'cancelled'] = '', period: str = '', cursor: int = 0) -> str:
    ...
    """
    from paa_server.modules.reports.schedule import obligation_page
