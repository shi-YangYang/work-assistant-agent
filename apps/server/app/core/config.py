import json
import os
from dataclasses import dataclass, field
from dotenv import load_dotenv
from pathlib import Path

ROOT = Path(__file__).resolve().parents[4]


load_dotenv(ROOT / 'apps/server/.env.web')


def resource_path(name: str, default: str) -> Path:
    # App-directory commands and root npm scripts use the same persistent data.
    return (ROOT / os.getenv(name, default)).resolve()


def model_options(name: str) -> dict:
    value = json.loads(os.getenv(name, '{}'))
    allowed = {'enable_thinking', 'thinking_budget', 'reasoning_effort'}
    if not isinstance(value, dict) or set(value) - allowed:
        raise ValueError(f'{name} contains unsupported model parameters')
    return value


@dataclass(frozen=True)
class Settings:
    database_url: str = field(repr=False, default_factory=lambda: os.getenv('DATABASE_URL', 'postgresql+psycopg://paa:local@127.0.0.1:5432/paa_company').replace('postgresql://', 'postgresql+psycopg://', 1))
    media_dir: Path = field(default_factory=lambda: resource_path('PAA_MEDIA_DIR', 'data/company/media'))
    web_origin: str = field(default_factory=lambda: os.getenv('PAA_WEB_ORIGIN', 'http://127.0.0.1:5174').rstrip('/'))
    login_company_id: str = field(default_factory=lambda: os.getenv('PAA_LOGIN_COMPANY_ID', '').strip())
    cookie_secure: bool = field(default_factory=lambda: os.getenv('PAA_COOKIE_SECURE', 'true').lower() == 'true')
    agent_base_url: str = field(default_factory=lambda: os.getenv('PAA_AGENT_BASE_URL', ''))
    agent_key: str = field(repr=False, default_factory=lambda: os.getenv('PAA_AGENT_API_KEY', ''))
    agent_model: str = field(default_factory=lambda: os.getenv('PAA_AGENT_MODEL', 'qwen3.5-flash-2026-02-23'))
    agent_options: dict = field(default_factory=lambda: model_options('PAA_AGENT_OPTIONS'))
    asr_base_url: str = field(default_factory=lambda: os.getenv('PAA_ASR_BASE_URL', ''))
    asr_key: str = field(repr=False, default_factory=lambda: os.getenv('PAA_ASR_API_KEY', ''))
    asr_model: str = field(default_factory=lambda: os.getenv('PAA_ASR_MODEL', 'qwen3-asr-flash'))
    model_key_file: Path = field(default_factory=lambda: resource_path('PAA_MODEL_KEY_FILE', 'data/company/model-master.key'))
    model_allowed_origins: tuple[str, ...] = field(default_factory=lambda: tuple(x.strip().rstrip('/') for x in os.getenv('PAA_MODEL_ALLOWED_ORIGINS', '').split(',') if x.strip()))
    ffmpeg: str = field(default_factory=lambda: os.getenv('PAA_FFMPEG', 'ffmpeg'))

    voiceprint_python: Path = field(default_factory=lambda: Path(os.getenv('PAA_VOICEPRINT_PYTHON', str(ROOT / '.venv-voiceprints' / ('Scripts/python.exe' if os.name == 'nt' else 'bin/python')))))
    voiceprint_model: Path = field(default_factory=lambda: Path(os.getenv('PAA_VOICEPRINT_MODEL', str(ROOT / 'apps/desktop/resources/models/speaker-community-1/embedding/pytorch_model.bin'))).resolve())

    worker_concurrency: int = field(default_factory=lambda: int(os.getenv('PAA_WORKER_CONCURRENCY', '3')))

    def __post_init__(self):
        if not 1 <= self.worker_concurrency <= 8:
            raise ValueError('PAA_WORKER_CONCURRENCY must be between 1 and 8')

    @property
    def checkpoint_url(self) -> str:
        return self.database_url.replace('postgresql+psycopg://', 'postgresql://', 1)
