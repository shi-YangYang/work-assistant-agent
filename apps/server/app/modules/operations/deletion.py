...
from app.modules.work.models import ProgressDraft, WorkRevision
from app.security.invalidation import invalidate_deleted as business_invalidate_deleted
...
async def remove_report(db, item, actor):
    ...
    item.revision += 1
    await db.flush()
    await business_invalidate_deleted(db, item.company_id)
