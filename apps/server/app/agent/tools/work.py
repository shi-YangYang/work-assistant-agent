...
from app.modules.work.projection import work_for_model as business_work_for_model
from app.modules.work.schemas import Progress
from app.security.access import inherit as business_inherit, require as business_require, valid as business_valid
...
async def find_work_items(query: str, runtime: ToolRuntime[RunContext], status: Literal['', 'in_progress', 'blocked', 'done'] = '', cursor: str = '') -> str:
    ...
    async with context.sessions.begin() as db:
        ...
        from app.modules.work.queries import work_search
        if query.strip():
