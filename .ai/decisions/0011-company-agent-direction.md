...
2026-09-15：用户确认 [Spec 017](../../specs/spec-017-report-reliability-and-reminders/spec.md) 的日报／周报提醒采用站内通知，员工在“我的报告”处理、管理员在团队看板查看提交情况；提醒节奏及可配置项见该 Spec R4。不接入外部消息服务，也不改变员工审阅后提交的边界。无已确认工作记录时仍需本人据实填写并提交，避免把“未录入”误判为免交或完成；具体待办行为见 R3。

2026-09-16：[Spec 018](../../specs/spec-018-web-pilot-experience/spec.md) 采用站内文字问题反馈和受限诊断摘要，由本公司管理员处理，首版不含截图或站外工单。反馈独立于业务消息及 Agent 记忆；手机优先适配 Safari／Chrome，继续使用同一套 Web。范围和验收边界统一在该 Spec 维护。
...
2026-09-14：管理员业务问答与本人督办进入 [Spec 014](../../specs/spec-014-admin-business-assistant/spec.md) 规划；服务端授权、派生回答及来源依赖的取舍见 [决策 0014](0014-team-assistant-authorization.md)。规划不代表这些能力已经实现，原始上报的具体可检索范围以该 Spec 的确认结果为准。
...
- 各端同仓库、独立运行／构建／部署，一次变更可共同审查接口与 UI，并保留桌面独立发布能力。最初在 `src/` 内扩展的目录安排已由 [0015](0015-multi-client-repository.md) 接续为 apps／services／packages 与 npm workspaces，仍不引入 Turborepo。
...
| Linux Docker Compose＋Caddy、API、单 worker 进程、PostgreSQL | 小规模单机起步；Spec 017 将初版串行处理改为有界并行，保留每成员串行及数据库租约，不增加消息队列服务 |
...
Spec 017 的报告整理改为受控结构化输出，服务端校验并保存，避免把模型是否调用保存工具作为任务完成条件；继续复用 harness 的权限、来源、配置快照和调用预算。聊天仍使用受控工具循环，报告流程简化不代表移除业务 harness。
