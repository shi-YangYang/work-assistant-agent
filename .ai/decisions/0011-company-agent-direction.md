...
- 管理员保留工作助手和本人工作空间；督办由本人确认，不代员工修改记录。会话历史独立，已确认工作可按权限跨会话检索。
...
- 采用 Deep Agents／LangGraph 复用上下文和工具循环，关闭业务 Agent 的子 Agent、命令执行、宿主文件及任意网络工具。员工输入不能扩大权限；模型只提建议，正式确认与发布由用户执行。
