...
- Spec 007 与菜单体验修复已随 `8e9240f` 推送到 `dev`。CI 发现资料测试缺少源码导入路径，当前已修复，取消 `PYTHONPATH` 后该文件 12 项通过；Windows 结果须以当前修复的远端检查为准。命令与原因归入 [实施报告](../../specs/spec-007-meeting-library/implementation.md)。继续遵循 [Spec 规则](../rules/spec-decision-workflow.md) 与 [分支规则](../rules/git-branch-workflow.md)，不重复检查或持续等待 CI。
