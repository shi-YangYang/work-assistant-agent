...
- `meeting_library.py` 在有界后台队列检索完整历史，日期按绝对时间比较，标题／转写／纪要仅匹配可见文本。返回每页最多 10 场及精确片段／纪要版本定位，SQL 查询有进度期限；页面通过请求代次处理旧响应，保留筛选与返回位置。
...
后续 CI 暴露 `test_meeting_library.py` 漏加源码导入路径；此前命令中的 `PYTHONPATH` 掩盖了该遗漏。现已与其他测试文件保持一致，在导入前按文件位置添加 `src/python`。修复后使用 `env -u PYTHONPATH .venv/bin/python -m unittest discover -s tests/python -p 'test_meeting_library.py' -v`，12 项通过（0.149 秒）；未重复其他套件，远端结果待推送后核对。
