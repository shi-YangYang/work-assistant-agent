...
这是小规模固定公开语料的本应用参考机实测，不是全量 FLEURS／ASCEND 榜单或真实公司会议准确率保证；未覆盖远场多人重叠、专业词表、口音全分布和所有噪声。没有 Windows 模型矩阵、其他 GPU 设备或最低配置实测，不外推 macOS 内存结果。

## 2026-09-15：medium GPU 增补

用户要求直接实测并填入模型信息。本轮使用同一台 M5／16 GiB、同一份冻结清单与三组完整组合音轨，未重选或裁剪样本。调用当前应用的 MLX Provider、语言设置、业务分块和评分函数；每语言独立进程，模型加载至评分后分别记录进程峰值 RSS 与 MLX GPU 分配峰值。GPU 使用 Apple 共享内存，两个统计范围不同，不能相加，也不代表整应用内存。

运行配置为 mlx-whisper 0.4.3／MLX 0.32.2、Metal FP16、greedy（beam 1），固定模型 `mlx-community/whisper-medium-mlx@7fc08c4eac4c316526498f147dfdee6f6303f975`。CPU 旧结果保持不变；其引擎、精度和 beam 5 与 GPU 配置不同，不能把差异仅归因于硬件。

| 语言 | 错误率 | 进程峰值内存 | GPU 峰值分配 |
| --- | --- | --- | --- |
| 中文 CER | 5.98%（15 替换＋5 删除＋2 插入／368） | 1.08 GB | 2.22 GB |
| 英文 WER | 5.33%（13 替换＋3 删除＋1 插入／319） | 1.00 GB | 2.21 GB |
| 中英混合 MER | 36.60%（131 替换＋35 删除＋6 插入／470） | 2.06 GB | 2.21 GB |
