...
用户于 2026-09-11 再次明确以下长期约束，后续任务和上下文恢复均须遵守；本节替代此前偏重的默认验证安排。
...
用户于 2026-09-11 最新补充：创建 Spec、讨论需求、技术选型、起草决策与相关文档时，由协调 Agent 直接完成，不创建调研、审查或验收子 Agent，也不安排独立验收。写完按 S0 轻量检查 diff、需求一致性和引用，再交用户审查；不为文档运行测试或构建。本约定从当前 Spec 立即生效。
...
用户于 2026-09-11 最新明确：征询意见只用普通文本对话，不使用问题卡；每轮集中询问多个当前需要确定的问题，同一个 Spec 可以有多轮讨论。本约定从本次协调 Agent 的 Spec 工作开始执行，并持续适用于后续任务。
...
用户于 2026-09-11 最新约定：长期在 `dev` 开发；用户要求提交并推送后，Agent 提供 `dev → main` 的 HTTP(S) PR 链接或 PR 创建页链接，由用户决定并执行合并。合并后由用户将最新 `main` 同步回 `dev`，也可另行要求 Agent 操作。取消此前 Agent 自动合并、自动回同步及持续等待 CI 的安排；`dev` 长期保留。用户要求仅本地 commit 或不要触发 CI 时，按该限制交付。具体见 [Git 分支工作规则](.ai/rules/git-branch-workflow.md)。
...
用户于 2026-09-10 明确要求：后续本机桌面功能测试、交互验证与验收，直接在项目目录执行 `npm run dev`，使用用户现有环境。本约定在任务切换和上下文恢复后持续适用。
...
用户于 2026-09-15 明确：README 面向用户阅读，优先说明功能、安装使用和技术栈，保持简洁；Logo 与徽章居中。详细部署、备份和排障步骤集中到使用指南，Agent 流程、验收记录和内部实现约束不堆进 README。
