...
用户于 2026-09-10 确认以下持续约定：

- 修改项目格式化器覆盖的源码文件时，即使只是文案或 JSX 调整，也要使用项目已安装的格式化器与配置整理本次改动文件，不凭肉眼判断换行。格式化属于编辑步骤，不默认格式化整个仓库。
- `git diff --check` 不能代替 Prettier 等格式检查。出现格式检查失败时，只修复并检查涉及的文件；检查通过且文件未再修改，不重复执行。
- 本地验证继续遵守上述 S0～S3 规则，选择与改动直接相关的最小必要检查；不能遗漏相关检查，也不因格式调整扩大为完整测试、build、lint 或 typecheck。
- 推送后核对该提交的远端 CI 状态，交付时区分本地验证、提交／推送结果和远端 CI 结果。尚未查询、未触发、仍在运行或失败时应如实说明；只有对应提交的所有必需检查成功，才能声明“CI 已通过”，不能用旧提交的结果代替。
...
用户已于 2026-09-10 明确要求：默认不单独创建开发分支；如 Agent 为任务创建了分支，验收合格后必须自动合并到 `main`、推送，并删除自己创建的本地及远端分支，将工作区切回 `main`。不能只把验收通过的分支交给用户而遗漏合并与清理，也不需要再次请求合并确认。除非用户后续明确改变要求，本规则持续适用。详见 [Git 分支收尾规则](.ai/rules/git-branch-workflow.md)。
...
用户的长期要求：从写作时就避免冗余，不等用户提醒再整理。优先更新已有文档，同一信息只在一处详述，其余引用；任务收尾时归并过程记录并清除过期状态。具体遵循 [文档精简规则](.ai/rules/spec-decision-workflow.md#文档精简)。
