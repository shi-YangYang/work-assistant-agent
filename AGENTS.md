...
- 用户于 2026-09-11 确认 CI 按风险分层：日常保留快速检查及关键受控流程，真实模型与完整安装包检查按相关改动或发布需要触发，不因新增 Spec 无条件叠加重测试。具体矩阵和触发方式只在 [README 的 CI 分层](README.md#ci-分层) 维护。新测试应优先固定输入与同步条件；不能以加长等待、重复跑到绿或放宽安全断言代替修复根因。
...
用户于 2026-09-11 将分支约定更新为：长期保留 `dev` 作为默认开发分支；改动在 `dev` 完成并验证，通过后创建或更新 `dev → main` 的 PR，必要检查通过后合并，再将 `main` 合并回 `dev` 并推送，工作区回到 `dev`。`main` 承接通过验收的改动，不默认直接开发或推送业务修改；`dev` 不作为临时分支删除。该约定替代此前默认在 `main` 开发及任务结束切回 `main` 的规则，正常 PR 合并和同步无需重复确认。操作顺序、检查与临时分支收尾见 [Git 分支工作规则](.ai/rules/git-branch-workflow.md)。
