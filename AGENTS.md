...
- CI 的目标是快速、可靠地发现集成错误，没有每次提交必须完整验收产品的要求。日常代码 CI 以格式、Lint、类型、快速单元／模块测试和普通构建为主；普通构建不等于生成并安装发行包。本地文档整理仍按 S0，只检查 diff 和引用。
- 平台相关改动补对应平台的定向检查。真实模型下载／推理、完整桌面流程、双平台打包及安装卸载默认放在发布前或手动触发；相关高风险修改确有必要时可提前执行对应项，不因新增 Spec 自动叠加全套检查。
- 真实麦克风、系统权限和录音效果在用户日常环境验收；录音状态、暂停／继续、保存等逻辑优先通过固定输入的模块测试验证。自动测试不得依赖用户密钥、真实会议或物理设备。
- 先减少默认必跑项目和重复触发，再考虑动态调度。避免同一批内容在 dev 推送、PR、main 合并及回同步时重复执行整套重检查；不能把复杂的路径选择、更多任务或更多轮次本身当作减负。
...
### 项目补充约定：Spec 决策由主 Agent 完成，实施按是否开 Spec 分工

创建 Spec、讨论需求、技术选型、起草决策与相关文档时，由协调 Agent 直接完成，不创建调研、审查或验收子 Agent，也不安排独立验收。写完按 S0 轻量检查 diff、需求一致性和引用，再交用户审查；不为文档运行测试或构建。

用户要求开 Spec 时，关键决策确认后的业务实施走多 Agent 流程：实施 Agent 开发，新的独立验收 Agent 验收。用户要求不开 Spec／直接修改时，由协调 Agent 自行完成分析、修改和相称验证，不创建子 Agent。不得把“开 Spec”解释为决策阶段也需要子 Agent。详见 [Spec 工作规则](.ai/rules/spec-decision-workflow.md)。
...
征询意见只用普通文本对话，不使用问题卡；每轮集中询问多个当前需要确定的问题，同一个 Spec 可以有多轮讨论。

- 提问前先核对已有说明与项目事实，整理影响范围、产品行为、外部条件和验收的关键问题。
- 当前一轮能识别的问题用普通文本在同一条消息中列出，让用户一起回答，可附简短建议；不调用问题卡工具，也不把整组问题包装成一张卡。
- 一次 Spec 允许根据答复和调研继续多轮讨论，不要求把所有问题硬塞进首轮，也不把已知的同组问题拆成逐个追问。已经确认的信息和可自行判断的工程细节不重复询问；等待答复期间继续不依赖答案的工作。
...
每次生成 Spec，必须在 `plan.md` 中附上改造后的目录树，并在对话中展示。只列本次涉及的目录和文件，标明新增、修改、移动或删除及其职责；没有目录调整时，也列出原位修改的文件。方案变化后同步更新目录树。
...
- 只记录会影响后续开发、且无法从代码直接看出的重要取舍，如架构边界、关键业务规则、兼容限制和被否决的重要方案。
- 优先更新已有文档；确需独立保存的决策才放入 `.ai/decisions/`。已有 Spec 或规则写清的内容，不再复制一份。
- 直接写结论，按需补一句理由或关键限制；几条短句能说清就不套固定模板。
- 不加记录日期、对应 Spec 编号、“用户已确认”“本轮实施”等过程说明，不维护讨论流水账。必要引用用内容名称，变更历史交给 Git。
- 普通修复、样式文案、实现细节、测试重跑和 commit／push 不写 Decision，也不默认同步 Spec、实施报告、验收报告和交接；只有对应内容实际失效或需要交接未完成事项时才更新。
- 验收仍如实区分通过、失败和未执行，保留必要证据及限制；具体采样时间、业务日期等影响结论的信息不属于冗余元数据。
...
所有 commit 标题使用 `类型: 中文描述`，可按需写为 `类型(范围): 中文描述`。根据改动选择 `feat`、`fix`、`docs`、`style`、`refactor`、`perf`、`test`、`build`、`ci`、`chore` 或 `revert`；标题描述和正文继续使用中文，清楚说明本次改动。例如：`fix: 修复语音重试弹窗无法关闭`、`feat: 支持多个工作会话`、`docs: 精简使用说明`。
...
Web 前端新增或调整代码遵循[架构说明中的模块边界](docs/architecture.md#web-前端组织)：业务代码就近组织，页面保持轻量，公共层不得反向依赖业务；保持唯一请求与身份处理入口。超过约 350 个非空行的业务 TSX 应检查职责，不用机械拆文件满足行数。

公司共享后端遵循[服务端组织与依赖边界](docs/architecture.md#公司服务端组织)：业务按模块维护，HTTP 与 Agent 复用业务用例；配置、ORM、任务上下文和租约不得反向依赖应用／harness 装配。超过约 400 个非空行的 Python 文件检查职责，不增加无实际用途的分层。

### 项目补充约定：统一 UI 设计基准

- 涉及 Web、Electron 或未来手机端的 UI／UX 工作，先阅读并遵循 [UI 设计基准与归档原型](docs/design/README.md)，以其中的中性黑白灰、组件层级和交互效果作为设计与视觉验收依据；不得自行换回蓝紫强调或另起一套视觉风格。
- 各端统一设计语言，独立维护实现：Web 使用 CSS Modules，Electron 维护自己的样式，手机端适配触摸与窄屏，不互相引用应用私有 CSS。
- 改造先覆盖 Web，其他端另行安排；翻书登录页保留。原型仅供视觉参考，正式实施保留已有业务能力、权限和真实接口，不引入示例数据或模拟成功逻辑。
- 色值及组件细节只在设计基准维护；重要视觉方向调整先与用户确认，再更新基准，不重复堆积设计文档。
...
重大技术选择按留痕规则记录，已有文档说明清楚时不另建 Decision。
...
本机桌面功能测试、交互验证与验收，直接在项目目录执行 `npm run dev:electron`，使用用户现有环境。

- 使用应用默认用户数据目录，沿用已保存的 API 服务配置、密钥、录音和已下载模型；不再另建隔离验收窗口、设置 `PAA_TEST_DATA_DIR` 或替换 `userData`。
- 不因测试环境缺少配置而要求用户重新填写密钥、复制录音或重复下载模型；密钥由应用使用，不读取、输出或复制明文密钥。
- 保留用户现有数据。既有单元测试与 CI 的临时数据机制不改为操作真实用户数据，含数据清理或故障注入的脚本不得指向用户日常数据目录。
...
文档只保留后续需要的信息，同一事实只写一处。小改动不默认产生文档工作；必要时按 [留痕规则](#10-重要决策留痕) 和 [文档精简规则](.ai/rules/spec-decision-workflow.md#文档精简) 更新已有内容。
...
README 面向用户阅读，优先说明功能、安装使用和技术栈，保持简洁；Logo 与徽章居中。详细部署、备份和排障步骤集中到使用指南，Agent 流程、验收记录和内部实现约束不堆进 README。
...
高风险动作遵守用户授权边界；会留下长期影响的取舍按留痕规则记录，不为每次操作另建报告。
