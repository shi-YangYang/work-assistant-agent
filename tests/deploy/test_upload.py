...
''')
    ...
    def test_full_then_incremental_reuses_legacy_source_and_preserves_old_release(self):
        ...
        (previous / 'apps/server').mkdir(parents=True)
        (previous / 'apps/server/.env.web').write_text('server-only configuration')
        ...
        for name in ('deleted.txt', '.env.company', 'apps/server/.env.web', '.release.env'):
