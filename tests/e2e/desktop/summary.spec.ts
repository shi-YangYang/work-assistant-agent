...
import { pythonCommand } from '../../../apps/desktop/src/main/python-command'
import type { ServiceDraft } from '../../../apps/desktop/src/shared/summary-contracts'
...
test('multi-service settings, custom reasoning, real HTTP checks, minutes sources and restart are portable', async () => {
  ...
  const { meetingId } = JSON.parse(
    execFileSync(command, [...args, 'tests/core/summary_fixture.py', root], { encoding: 'utf8' }),
  ) as { meetingId: string }
  ...
  const launch = (): Promise<ElectronApplication> =>
    electron.launch({ args: [resolve('apps/desktop')], env })
