...
async function launch(
  ...
  if (synthetic) {
    ...
    writeFileSync(
      shim,
      `#!/bin/sh\nexec ${quote(resolve('.venv/bin/python'))} ${quote(resolve('tests/core/smoke_core.py'))} "$@"\n`,
    )
  ...
  return electron.launch({
    args: ['--use-fake-device-for-media-stream', resolve('apps/desktop')],
    env,
  })
}
...
sys.path.insert(0, str(Path('apps/desktop/core/src').resolve()))
