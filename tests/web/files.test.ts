...
describe('document composer', () => {
  it('accepts all supported documents with mixed images and enforces aggregate boundaries', () => {
    for (const extension of ['pdf', 'docx', 'pptx', 'txt', 'json', 'md', 'csv']) {
    ...
    expect(fileSelectionError([file('audio.m4a'), file('text.txt')])).toContain('一段语音')
    ...
    for (const extension of ['doc', 'ppt', 'xlsx', 'docm', 'exe'])
