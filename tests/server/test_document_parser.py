...
from paa_server.integrations.parsing.document_parser import MAX_EXPANDED
...
async def test_timeout_reaps_real_child_and_resource_guard(tmp_path):
    parser_path = Path(__file__).parents[2] / 'apps/server/src/paa_server/integrations/parsing/document_parser.py'
...
async def test_windows_job_object_limits_and_fail_closed(monkeypatch, assign_ok):
    ...
    from paa_server.integrations.parsing.document_parser import windows_resource_limits
    calls = []
    class Function:
