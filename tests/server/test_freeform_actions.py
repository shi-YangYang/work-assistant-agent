...
from paa_server.models import Job, Report, WorkRevision
...
async def test_business_planning_default_preserves_explicit_reasoning_and_unknown_providers():
    from paa_server.model_provider import business_model_config
...
async def test_incomplete_intent_model_response_is_not_reported_as_bad_user_fields(setup):
    from paa_server.business_actions import IntentCheckFailed
...
async def test_report_rewrite_judge_gets_original_facts_and_cannot_save_rejected_milestone(setup):
    from paa_server.agent.action_tools import query_reports
...
async def test_intent_request_uses_short_verification_without_changing_user_config(setup, monkeypatch):
    ...
    from paa_server.model_services import bind_job
    ...
    monkeypatch.setattr('paa_server.model_provider.client', lambda _: httpx.AsyncClient(transport=httpx.MockTransport(respond)))
