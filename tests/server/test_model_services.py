...
import httpx
import json
import paa_server.modules.model_services.probes as model_services
import pytest
...
from paa_server.integrations.models.transport import CheckedBackend, ProviderError, SafeTransport, allowed_address, normalize_url
from paa_server.modules.model_services.bindings import bind_job, resolve_bound
from paa_server.modules.model_services.models import ModelService, ModelServiceRevision, ModelUsage
from paa_server.modules.model_services.parameters import reply_review_config, request_options
from paa_server.modules.model_services.probes import reserve_probe as model_services_reserve_probe
from paa_server.modules.model_services.schemas import ServiceInput, parameters
from paa_server.security.secrets import SecretUnavailable, decrypt, initialize_key
...
async def test_cross_company_csrf_secret_failure_and_environment_takeover(setup):
    ...
    async with sessions.begin() as db:
        outsider=await db.get(__import__('paa_server.modules.members.models',fromlist=['Member']).Member, users['outsider'].id)
        outsider.role='admin'
...
async def test_streamed_tools_are_complete_and_audio_protocols_are_distinct(setup,monkeypatch):
    ...
    monkeypatch.setattr('paa_server.integrations.models.transport.client',lambda settings:httpx.AsyncClient(transport=httpx.MockTransport(response)))
    ...
    wav=(Path(__file__).parents[2]/'apps/server/src/paa_server/assets/probe-zh.wav').read_bytes()
...
async def test_actual_bounded_harness_uses_frozen_service_and_reserves_each_call(setup,monkeypatch,streaming,fast_review):
    ...
    monkeypatch.setattr('paa_server.tasks.feedback.publish', track_phase)
    monkeypatch.setattr('paa_server.tasks.handlers.publish', track_phase)
    reply='请确认这条进展建议。'
...
async def test_admin_probe_and_directory_validate_real_capabilities_without_business_writes(setup,monkeypatch):
    ...
    async with sessions() as db:
        ...
        from paa_server.modules.model_services.models import ModelCheck
...
async def test_token_plan_asr_explains_incompatible_model_before_request(monkeypatch, model, protocol):
    from paa_server.core.config import Settings
    calls = []
    monkeypatch.setattr('paa_server.integrations.models.transport.client', lambda settings: calls.append(settings))
...
async def test_explicit_environment_import_preserves_behavior_and_clear_never_falls_back(setup):
    from paa_server.modules.model_services.service import import_environment, routing_dto
    settings,sessions,users,c=setup
...
async def test_directory_official_pagination_stays_same_origin_and_rejects_response_compression(setup,monkeypatch):
    from paa_server.integrations.models.catalog import catalog
    settings,*_=setup
    calls=[]
    def response(request):
        calls.append(request)
...
async def test_company_probe_concurrency_is_bounded_without_daily_quota(setup,monkeypatch):
    from paa_server.modules.model_services.probes import reserve_probe
    settings,sessions,users,c=setup
