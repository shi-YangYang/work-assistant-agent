...
async def setup(tmp_path):
    ...
    settings = replace(base, database_url=test_url.replace('postgresql://', 'postgresql+psycopg://', 1), media_dir=tmp_path / 'media', cookie_secure=False, web_origin='http://test', agent_base_url='', agent_key='')
    ...
    async with sessions.begin() as db:
        for name in ('primary', 'other'):
            company = Company(name='受控测试公司 ' + uuid4().hex[:8])
