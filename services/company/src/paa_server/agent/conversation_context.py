"""Shared, bounded conversation references for reasoning and intent checking."""
import json
from sqlalchemy import select
from .. import business_access as business
from ..models import Job, Message
from ..service import active_message
...
async def conversation_references(db, actor, job, current, budget=12000):
    ...
    for row in rows:
        ...
        if size > budget:
            continue
        budget -= size
        job.access = business.merge_access(job.access or business.scope(actor), row.access)
