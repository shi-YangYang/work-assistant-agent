...
已有环境升级到 `0003_conversations` 时，先停止开发服务并备份数据库，再运行 `npm run db:company`、`npm run dev:company`。迁移将旧消息归入各自的“默认会话”，保留原消息、附件和业务引用；不需要重新创建账号或填写模型配置。生产备份与恢复方法见下方部署说明。
