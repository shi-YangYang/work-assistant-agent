...
设置中的“模型服务管理”“本地转写模型”和“录音与转写”可以独立折叠，记住上次展开状态。录音可暂停后继续，暂停时关闭麦克风且不补静音，内容追加到同一条音频；结束后可用自定义播放器拖动定位、前后跳转 10 秒、调节倍速和音量，快捷键说明在播放器内。
...
在“设置 → 模型服务管理”添加服务，填写包含版本路径的 API Base URL（例如 `https://example.com/v1`）和密钥。“获取模型”读取该服务返回的目录，可搜索并选择，也可手动填写模型 ID。目录可见不代表已取得生成权限；“测试连接”会用当前表单发送一次小型生成请求，可能产生 API 费用。
...
转写任务、已完成块和片段存入同一个 SQLite 数据库，模型位于用户数据目录的 `models/whisper-small-<revision>/`。当前使用 schema 4，旧版本依次增量迁移；迁移前生成 `meetings.schema<旧版本号>.backup.sqlite3`（支持 schema 1、2、3），失败保留原库并回滚。备份不含独立 WAV，手动备份应关闭应用后复制整个用户数据目录。旧程序不能直接读取 schema 4；若需回退，应关闭应用、保存完整当前目录，再使用升级前的数据库与对应音频副本，不能直接修改版本号或删表。
