...
assert.equal(
  ...
  readFileSync('apps/desktop/core/requirements.lock', 'utf8'),
)
...
try {
  ...
  const result = { inference: false }
  if (!process.argv.includes('--runtime-only')) {
    const fixture = resolve('artifacts/spec003/real-asr')
    const models = join(fixture, 'models')
    const directory = readdirSync(models).find((name) =>
      existsSync(join(models, name, 'model.bin')),
    )
    ...
    Object.assign(result, JSON.parse(checked.stdout.trim()))
  ...
  writeFileSync(
    `artifacts/spec005/package-${process.platform}-${process.arch}${process.argv.includes('--runtime-only') ? '-runtime-only' : ''}.json`,
